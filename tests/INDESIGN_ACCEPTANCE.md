...
首次实机运行在 `namePrefix = "A"` 处失败。修复将默认母版的移除提前到第一组新双页母版创建后、正式命名前；不让母版集合暂时为空。自动测试已加入默认 A-Parent 的重名检查，旧代码失败、修复后通过。此测试替身不代替实机验收。

重新运行入口，确认不再出现 namePrefix 错误，最终仅有上述 9 组母版、每组 2 页，初始页应用 A-TEXT；实机复测待用户完成。
