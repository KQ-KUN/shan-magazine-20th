...
(function () {
 ...
 try {
  ...
  var result=SHAN.interview.create(doc,File("__ROOT__/manuscripts/01_interview_邵珠瑜_贾锦阳.docx"),"interview_shao",context);
  if(result.overset || result.pages < 2 || result.story.paragraphs.length !== 41){throw new Error("Unexpected native layout result");}
  if(result.counts.InterviewQuestion!==13 || result.counts.InterviewAnswer!==24){throw new Error("Unexpected Q/A counts");}
  ...
  if(counts.P_Interview_Q!==13||counts.P_Interview_A!==24||counts.P_Article_Title!==1||counts.P_Article_Subtitle!==1||counts.P_Author!==1||counts.P_Metadata!==1){throw new Error("Wrong final mapped styles");}
  if(app.wordRTFImportPreferences.properties.toSource()!==oldPrefs){throw new Error("Import preferences not restored");}
  return "PASS InDesign "+app.version+": "+doc.extractLabel("SHAN_INTERVIEW_REPORT")+"; Q=13; A=24; mapped styles/2 columns/B-INTERVIEW/graphics=0/preferences restored";
