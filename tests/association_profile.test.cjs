...
assert.equal(tokens.ending.year_tint_percent, 40);
...
assert.ok(tokens.layout.year_top_mm - (tokens.layout.body_top_mm + 5 * tokens.body.leading_pt * 25.4 / 72) >= 13);
assert.ok(tokens.layout.year_top_mm - (tokens.layout.body_top_mm + 5 * tokens.body.leading_pt * 25.4 / 72) <= 19);
