...
- 山口无编号文本框，其他页面显示 manifest 的 display_index：壹、贰、叁、肆、伍、陆，不显示阿拉伯数字或“·”；chapter_index 仍为数字 1—6。中英文标题、前言逐字与 manifest 一致。
- 文本框 label 为 `SHAN_CHAPTER:<section.id>:<field>`；编号框字段为 display_index；字段分别绑定既有 P_Chapter_Number、P_Section_Title_CN、P_Section_Title_EN、P_Chapter_Intro。全篇不做局部字体或字号格式化。
...
用户已确认上一版本的 7 页结构测试成功、无 API 报错。本次切换显示编号后需重新验收，Chapter 按要求继续保持 IMPLEMENTED_PENDING_IND2026_TEST，不冻结。所有非冻结 JSX 的 BOM 由 Chapter 自动测试检查，冻结文件单独与 foundation-v1.0 比较。
