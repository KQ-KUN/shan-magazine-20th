...
SHAN.featureSkin = {
    ...
    contains: function (items, value) {
        var i;
        for (i = 0; i < items.length; i += 1) { if (items[i] === value) { return true; } }
        return false;
    },
    apply: function (doc, t, base, context) {
        ...
        doc.paragraphStyles.itemByName("P_Feature_Title").justification = Justification.CENTER_ALIGN;
        doc.paragraphStyles.itemByName("P_Feature_Author").justification = Justification.CENTER_ALIGN;

        style = doc.paragraphStyles.itemByName("P_Feature_Media");
        style.spaceBefore = SHAN.utils.pt(t.media.space_before_mm);
        style.spaceAfter = SHAN.utils.pt(t.media.space_after_mm);
        ...
        for (i = 0; i < names.length; i += 1) {
            ...
            if (this.contains(t.span_styles, names[i])) {
                style.spanColumnType = SpanColumnTypeOptions.SPAN_COLUMNS;
                style.spanSplitColumnCount = t.body_columns;
            } else {
