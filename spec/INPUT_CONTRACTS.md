...
每个 section 必须包含：
```json
{"id":"origin","chapter_index":1,"display_index":"壹","cn":"火种","en":"ORIGIN","intro":"..."}
```
字段：`id` 为稳定机器标识；`chapter_index` 山口为 null、火种至山外为数字 1—6，仅用于机器排序与校验；`display_index` 山口为 null、火种至山外依次为壹、贰、叁、肆、伍、陆，用于页面编号显示，不含“·”；`cn` 中文章节名；`en` 英文章节名；`intro` 章节前言，原文使用，不改写。
